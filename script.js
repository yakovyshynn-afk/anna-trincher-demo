...
(function () {
  ...
  /* ===== БЛОК: COSMIC FIELD — наскрізний фон (canvas) =====
     Той самий рецепт, що initStarfield вище (procедурні зорі,
     розмір/яскравість/мерехтіння випадкові), але: 1) прив'язаний до
     viewport (window.innerWidth/innerHeight), не до батьківського
     елемента — сам шар position:fixed, тому розмір canvas завжди =
     розмір вікна; 2) значно розрідженіший (менше зірок, менша базова
     яскравість) — фоновий ambient-шар для ВСІХ секцій одночасно, не
     самостійний "герой"-ефект, як у hero. Один canvas на всю сторінку
     (а не по одному на секцію) — дешевше й простіше за FPS-бюджетом. */
  (function initCosmicField() {
    var canvas = document.getElementById("cosmicStars");
    if (!canvas || !canvas.getContext) return;
    var ctx = canvas.getContext("2d");
    var stars = [];
    var dpr = Math.min(window.devicePixelRatio || 1, 2);
    ...
    function buildStars() {
      var area = width * height;
      var count = Math.max(70, Math.min(200, Math.round(area / 9000)));
      stars = [];
      for (var i = 0; i < count; i++) {
    ...
    function resize() {
      /* getBoundingClientRect на .cosmic-field (батько), не
         window.innerWidth/innerHeight — той самий прийом, що
         initStarfield вище (canvas.parentElement.getBoundingClientRect),
         аби уникнути розбіжності large/small viewport на мобільних
         браузерах (position:fixed сайзиться по-іншому, ніж window.inner*,
         у деяких мобільних UA при появі/сховуванні адресного рядка). */
      var rect = canvas.parentElement.getBoundingClientRect();
      width = rect.width;
      height = rect.height;
      canvas.width = Math.round(width * dpr);
